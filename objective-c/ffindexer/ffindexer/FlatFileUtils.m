...
-(void)parse
{
    ...
    NSCharacterSet * ancs = [NSCharacterSet alphanumericCharacterSet];
//    NSCharacterSet * wscs = [NSCharacterSet whitespaceCharacterSet];
    NSCharacterSet * rmcs = [NSCharacterSet characterSetWithCharactersInString:@"<>:.;\"\'+-,"];

    NSMutableData * uniArrData = [[NSMutableData alloc] initWithCapacity:([self.text length] + 1)*sizeof(unichar)];
    [uniArrData setLength:[self.text length]];
    unichar * uniArr = uniArrData.mutableBytes;
    [self.text getCharacters:uniArr];
    
    for(NSUInteger pi = 0; pi < [self.text length]; pi++)
    {
        if ([ancs characterIsMember:uniArr[pi]]) {
        } else if ([rmcs characterIsMember:uniArr[pi]]) {
        } else {
            uniArr[pi] = ' ';
        }
    }
    
    NSData * data = [[[NSString alloc] initWithCharacters:uniArr length:[self.text length]] dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
