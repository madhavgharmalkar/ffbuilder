...
./bin/sqliteloader -table words -cols "idx:text,uid:integer,word:text,indexbase:integer,data:blobfile" -i words_b.txt -t vb2014.ivd -noclean
./bin/sqliteloader -table docinfo -cols "name:text,valuex:text,idx:integer" -i docinfo.txt -t vb2014.ivd
./bin/sqliteloader -table objects -cols "objectName:text,objectData:blobfile,objectType:text" -i objects.txt -t vb2014.ivd
./bin/sqliteloader -table groups -cols "groupname:text,groupid:integer" -i groups.txt -t vb2014.ivd
./bin/sqliteloader -table groups_detail -cols "groupid:integer,recid:integer" -i groups_detail.txt -t vb2014.ivd
./bin/sqliteloader -table texts -cols "recid:integer,plain:text,levelname:text,stylename:text" -i texts-b.txt -t vb2014.ivd
./bin/sqliteloader -table levels -cols "id:integer,original:text,level:text" -i levels.txt -t vb2014.ivd
./bin/sqliteloader -table popup -cols "title:text,class:text,plain:text" -i popup.txt -t vb2014.ivd
./bin/sqliteloader -table jumplinks -cols "title:text,recid:integer" -i jumplinks.txt -t vb2014.ivd
./bin/sqliteloader -table styles -cols "id:integer,name:text" -i styles.txt -t vb2014.ivd
./bin/sqliteloader -table styles_detail -cols "styleid:integer,name:text,valuex:text" -i styles_detail.txt -t vb2014.ivd
